            ...
            for k, data in enumerate(self.train_loader):
                ...
                with autocast():
                    ...
                    if self.mode == "mst":
                        ...
                        loss_lwf = LwF_KL(logits_per_image, logits_base)

                    loss = criterion_XE(logits_per_image, L).mean()
                    
                optimizer.zero_grad()
                total_loss = loss + lwf_alpha * loss_lwf
                ...
                self.writer.add_scalars('loss_compare', 
                    ...
                    'XE_loss': loss.data, 
                    'loss_lwf': loss_lwf.data,
                    }, 
                    global_step=global_step)
                if self.mode == "mst":
                    print('Gstep: %6d; epoch: %d; [%5d] loss: %.3f (%.3f, %.3f); learning_rate: %f' 
                            % (global_step, (self.epoch*self.phase+j), k, total_loss.data, loss.data, loss_lwf.data,
                            optimizer.param_groups[0]['lr']))
                else:
                    print('Gstep: %6d; epoch: %d; [%5d] loss: %.3f (%.3f, %.3f); learning_rate: %f' 
                            % (global_step, j, k, total_loss.data, loss.data, loss_lwf.data,
                            optimizer.param_groups[0]['lr']))
                    lr_scheduler.step()
