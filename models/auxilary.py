    ...
    if not torch.jit.is_scripting():
        ...
        if any([type(t) is not Tensor for t in tens_ops]) and F.has_torch_function(tens_ops):
            return F.handle_torch_function(
                ...
                bias_k, bias_v, add_zero_attn, dropout_p, out_proj, training=training, key_padding_mask=key_padding_mask,
...
class MultiheadAttention(torch.nn.Module):
    ...
    def forward(self, query, key, value, out_proj_weight=None, key_padding_mask=None,
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            return multi_head_attention_forward(
                ...
                self.dropout, self.out_proj,#out_proj_weight, self.out_proj.bias,
            ...
            return multi_head_attention_forward(
                ...
                self.in_proj_weight, self.in_proj_bias,
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj,#out_proj_weight, self.out_proj.linear.bias,
                training=self.training,
                key_padding_mask=key_padding_mask, need_weights=need_weights,
                attn_mask=attn_mask,
                attention_probs_forward_hook=attention_probs_forward_hook,
